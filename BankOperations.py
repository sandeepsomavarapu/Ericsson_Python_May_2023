...
def validate_num(num):
    ...
    while flag<0:
         matches = re.findall(r'[7-9]\d{5}', num)
         if not matches:
             num = input("Invalid phone number. Please enter a n10 digit phone number starting with 7,8 or 9.\n\n---------------:")
         else:
            return num
            flag=10
...
con = mysql.connector.connect(host="localhost",user="root",passwd="rpsconsulting",database="accenture")
cursor = con.cursor()
...
#cursor.execute("create table txn (acc_num number(10), dt date not null, amount BINARY_FLOAT not null, d_or_c varchar2(1),CONSTRAINT fk_customer_ACC_NUM FOREIGN KEY (ACC_NUM) REFERENCES customer (ACC_NUM))") 
    ...
    if choice == 1:
        ...
        for res in results:
            if res[4] == acc_num: #if there's a match
                ...
                con.commit()
                trans_query=("INSERT INTO TXN(ACC_NUM, DT, AMOUNT,D_OR_C) VALUES(%s, %s, %s, %s)") #insert row in txn
                cursor.execute(trans_query, [acc_num, date.today(), dep_amt, 'Credit'])
                con.commit()
        ...
        for res in results:
            if res[4] == acc_num: #if there's a match
                ...
                cursor.execute(trans_query, [acc_num, date.today(), with_amt, 'Debit'])
        ...
        for res in results:
            ...
            if a1cnt==1 and a2cnt==1: #if both account numbers are present
                ...
                trans_query="INSERT INTO TXN(ACC_NUM, DT, AMOUNT,D_OR_C) VALUES(%s, %s,%s, %s)"
                cursor.execute(trans_query, [acc1, date.today(), amt, 'Debit'])
                con.commit()
...
# create table customer(fname varchar(20),lname varchar(20),phone_num int(15),bal int(15),acc_num int(15));
# create table txn(acc_num int,dt date,amount int,D_OR_C varchar(20));